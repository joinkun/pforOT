testthat::test_that("barycentric_projection works, p = 2 tensor", {
  ...
  testthat::expect_silent(fit <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights))
  ...
  fit2 <- pforOT:::barycentric_projection(y ~ ., data = df, weight = w)
  fit3 <- fit
  fit3$data@weights <- w
  testthat::expect_equal(fit3, fit2)
  
  # for same data
  preds <- pforOT:::predict.bp(fit)
  ...
  mess <- testthat::capture_output(fito <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = power, cost.online = "online"))
  mess <- testthat::capture_output(predo <- predict(fito))
  testthat::expect_equal(preds, predo, tol = 1e-5)
  
  # compare to manual est
  y_hat <- rep(NA_real_, pforOT:::get_n(fit$data))
  y0 <- pforOT:::get_y0(fit$data)
  y1 <- pforOT:::get_y1(fit$data)
  x0 <- pforOT:::get_x0(fit$data)
  x1 <- pforOT:::get_x1(fit$data)
  z  <- pforOT:::get_z(fit$data)
  c_pot <- fit$potentials$f_aa
  t_pot <- fit$potentials$g_bb
  C_00  <- pforOT:::cost(x0,x0, power, cost_function = fit$cost_function)
  C_11  <- pforOT:::cost(x1,x1, power, cost_function = fit$cost_function)
  lambda <- fit$penalty
  w0_log  <- pforOT:::log_weights(weights@w0)
  w1_log  <- pforOT:::log_weights(weights@w1)
  ...
  preds2 <- pforOT:::predict.bp(fit, newdata=df.new, source.sample = data$get_z())
  testthat::expect_equal(preds, preds2, tol = 1e-5)
  preds2o <- pforOT:::predict.bp(fito, newdata=df.new, source.sample = data$get_z())
  ...
  data$gen_data()
  df.new <- data.frame(y = data$get_y(), z = data$get_z(), data$get_x())
  testthat::expect_silent(preds3 <- pforOT:::predict.bp(fit, newdata=df.new, source.sample = data$get_z()))
  
  fit_debias <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = power, debias = TRUE)
...
testthat::test_that("barycentric_projection works, p = 1", {
  ...
  weights <- pforOT::calc_weight(x = data,
                                   ...
                                   method = "NNM")
  df <- data.frame(y = data$get_y(), z = data$get_z(), data$get_x())
  testthat::expect_silent(fit <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = power))
  testthat::expect_equal(fit$a, weights@w0)
  testthat::expect_equal(fit$b, weights@w1)
  testthat::expect_equal(fit$p, power)
  ...
  # check fit works if provide  weights as a vector
  w <- c(weights@w0, weights@w1)[order(order(data$get_z()))]
  fit2 <- pforOT:::barycentric_projection(y ~ ., data = df, weight = w, p = power)
  fit3 <- fit
  fit3$data@weights <- w
  testthat::expect_equal(fit3, fit2)
  ...
  testthat::expect_warning(fito <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = power, cost.online = "online"))
...
testthat::test_that("barycentric_projection works, p = 3", {
  ...
  # check fit works if provide  weights as a vector
  w <- c(weights@w0, weights@w1)[order(order(data$get_z()))]
  fit2 <- testthat::expect_silent(pforOT:::barycentric_projection(y ~ ., data = df, weight = w, p = power))
  fit3 <- fit
  fit3$data@weights <- w
  testthat::expect_equal(fit3, fit2)
  
  # for same data
  testthat::expect_warning(preds <- pforOT:::predict.bp(fit))
  ...
  mess <- testthat::capture_output((fito <- pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = power, cost.online = "online")))
  testthat::expect_error(pforOT:::barycentric_projection(y ~ ., data = df, weight = weights, p = as.numeric(power), cost.online = "online"))
  ...
  # for new 
  df.new <- data.frame(y = data$get_y(), z = data$get_z(), data$get_x())
  testthat::expect_warning(preds2 <- pforOT:::predict.bp(fit, newdata=df.new, source.sample = data$get_z()))
  testthat::expect_equal(preds, preds2)
  
  data$gen_data()
  df.new <- data.frame(y = data$get_y(), z = data$get_z(), data$get_x())
  testthat::expect_warning(preds3 <- pforOT:::predict.bp(fit, newdata=df.new, source.sample = data$get_z()))
