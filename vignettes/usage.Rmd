...
vignette: >
  %\VignetteIndexEntry{Using pforOT}
...
To use this function outside of the main pforOT functions, we would do
...
#### 1. Effective sample size
Typically, estimated samples sizes with weights are calculated as $\sum_i 1/w_i^2$ and gives us a measure of how much information is in the sample. The lower the effective sample size (ESS), the higher the variance, and the lower the sample size, the more weight a few individuals have. Of course, we can calculate this in `pforOT`!
...
We offer such checks in `pforOT` as well.
